...
| 动态路况闭环 | 影像分析后更新道路状态，并触发路线自动改道 | Demo Mock |
...
MVP 的影像分析由稳定 Mock 驱动，但业务层只依赖统一的 `analyzeRoadMedia(file, profile, context, onProgress)` 接口，可替换为真实多模态模型服务。
...
- 影像分析使用确定性的 Demo Mock，尚未接入真实 VLM 服务。
...
当前 Demo 在浏览器本地处理所选择的文件名和模拟分析流程，不会将媒体上传到远程服务。接入真实影像分析服务前，必须补充用户授权、隐私说明、数据最小化、保留周期和删除机制。
